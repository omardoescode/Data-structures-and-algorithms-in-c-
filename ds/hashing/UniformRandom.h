// https://users.cs.fiu.edu/~weiss/dsaa_c++4/code/UniformRandom.h
...
public:
   ...
   */
  int nextInt(int high) { return static_cast<int>(abs(nextLong() % high)); }
   ...
   */
  int nextInt(int low, int high) { return nextInt(high - low + 1) + low; }
